...
Один из наиболее удобных вариантов авторизации теперь происходит через Personal Access Token (PAT).

. Перейдите на сайт GitHub в настройки профиля: Settings -> Developer settings -> Personal access tokens -> Tokens (classic).
. Нажмите Generate new token (classic), выберите права доступа (repo) и скопируйте созданный токен.
. В терминале при выполнении команды `git push` в качестве Username введите ваш логин на GitHub, а в качестве Password вставьте скопированный токен.
