...
+
NOTE: Этот шаг нужен только в том случае, если вы задали кодовую фразу, link:https://wiki.archlinux.org/title/SSH_keys#SSH_agents[ssh-agent^] сохраняет ключ в памяти сессии, чтобы вам не приходилось вводить фразу вручную при каждом обращении к GitHub
+
[source,shell]
----
eval "$(ssh-agent -s)"
