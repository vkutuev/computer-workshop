...
NOTE: Утилита GitHub CLI автоматически настраивает кэширование персонального токена доступа (PAT) в Git Credential Manager, поэтому вам не придётся вводить данные вручную при каждой операции.
...
TIP: Нажмите `Enter`, чтобы использовать путь по умолчанию (`~/.ssh/id_ed25519`). При необходимости задайте кодовую фразу (_passphrase_)
